...
class SafetyTests(TestCase):
    def setUp(self):
        ...
        self.rule=ComplianceRule.objects.create(jurisdiction=self.city,version=1,reviewed=True,required_credentials=['identity'],required_vaccines=['rabies'])
        self.provider=Provider.objects.create(owner=self.operator,name='Boarder',jurisdiction=self.city,kind='facility',locality='Test locality',capacity=1,species=['dog'])
        self.pet=Pet.objects.create(owner=self.owner,name='Uno',species='dog')
        self.start=timezone.localdate();self.end=self.start+timedelta(days=2)
        Credential.objects.create(provider=self.provider,category='identity',status='verified',expires=self.end,verified_by=self.operator,verified_at=timezone.now())
        HealthRecord.objects.create(pet=self.pet,kind='rabies',status='verified',expires=self.end,verified_by=self.operator,verified_at=timezone.now())
    def reserve(self): return reserve(self.owner,self.provider.pk,[self.pet.pk],self.start,self.end)
    def test_expired_health_blocks_and_rolls_back(self):
        self.pet.records.update(expires=self.start)
        with self.assertRaises(ValidationError): self.reserve()
    ...
    def test_resident_cat_is_hard_exclusion(self):
        self.provider.resident_species=['cat'];self.provider.save()
        with self.assertRaises(ValidationError): self.reserve()
    def test_medication_competency_is_hard_exclusion(self):
        self.pet.medication_required=True;self.pet.save()
        with self.assertRaises(ValidationError): self.reserve()
    ...
    def test_unknown_jurisdiction_fails_closed(self):
        self.rule.reviewed=False;self.rule.save()
        with self.assertRaises(ValidationError): self.reserve()
    def test_home_boarding_prohibited(self):
        self.provider.kind='home';self.provider.save()
        with self.assertRaises(ValidationError): self.reserve()
    ...
    def test_pet_ownership(self):
        with self.assertRaises(PermissionDenied): reserve(self.stranger,self.provider.pk,[self.pet.pk],self.start,self.end)
    def test_multiple_pets_each_checked(self):
        self.provider.capacity=3;self.provider.save()
        other=Pet.objects.create(owner=self.owner,name='Other',species='dog')
        with self.assertRaises(ValidationError): reserve(self.owner,self.provider.pk,[self.pet.pk,other.pk],self.start,self.end)
    def test_cancellation_releases_once(self):
        b=self.reserve();cancel(self.owner,b.pk);cancel(self.owner,b.pk)
        self.assertEqual(CapacityDay.objects.first().reserved,0)
        self.reserve()
    def test_care_is_attributed_and_idempotent(self):
        b=self.reserve();b.status='active';b.save()
        t=CareTask.objects.create(booking=b,pet=self.pet,kind='Feed',due_at=timezone.now())
        with self.assertRaises(PermissionDenied): complete_task(self.owner,t.pk)
        complete_task(self.operator,t.pk);complete_task(self.operator,t.pk)
    ...
    def test_incident_hold_and_critical_notifications(self):
        b=self.reserve()
        with self.assertRaises(PermissionDenied): report_incident(self.stranger,b.pk,'injury','Observed injury')
    ...
    def test_critical_escalation_is_idempotent(self):
        b=self.reserve();b.status='active';b.save()
        CareTask.objects.create(booking=b,pet=self.pet,kind='Medicine',due_at=timezone.now()-timedelta(minutes=5),critical=True)
        self.assertEqual(escalate_overdue(),1)
        self.assertEqual(escalate_overdue(),0)
    def test_evidence_instance_immutable(self):
        b=self.reserve();e=CareEvent.objects.create(booking=b,actor=self.owner,kind='Test')
        e.notes='Changed'
        with self.assertRaises(ValueError): e.save()
    def test_non_participant_cannot_read_stay(self):
        b=self.reserve();self.client.force_login(self.stranger)
    ...
    def test_parent_pages_render(self):
        b=self.reserve();self.client.force_login(self.owner)
        for url in ['/','/explore/','/bookings/','/profile/','/messages/',f'/stays/{b.pk}/']:
    ...
    def test_search_displays_only_eligible(self):
        self.client.force_login(self.owner)
        response=self.client.get('/explore/',{'city':'Bengaluru','starts':self.start.isoformat(),'ends':self.end.isoformat(),'pets':[self.pet.pk]})
        self.assertEqual(response.status_code,200)
        self.assertContains(response,'Boarder')
    def test_handover_requires_both_parties(self):
        b=self.reserve();b.status='confirmed';b.save()
        acknowledge_handover(self.operator,b.pk,'Alert and comfortable','Bed','Food',True)
        b.refresh_from_db();self.assertEqual(b.status,'confirmed')
        acknowledge_handover(self.owner,b.pk)
        b.refresh_from_db();self.assertEqual(b.status,'active')
