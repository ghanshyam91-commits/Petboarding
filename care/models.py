...
class HealthRecord(models.Model):
    ...
    status = models.CharField(max_length=16,choices=[(x,x.title()) for x in ['unverified','pending','verified','expired','rejected']],default='pending')
...
class Provider(models.Model):
    ...
    jurisdiction = models.ForeignKey(Jurisdiction,on_delete=models.PROTECT)
    kind = models.CharField(max_length=20,choices=[('facility','Facility'),('home','Home boarding'),('daycare','Daycare'),('sitter','Pet sitter')])
...
class Credential(models.Model):
    provider = models.ForeignKey(Provider,on_delete=models.PROTECT,related_name='credentials')
...
class StaffMembership(models.Model):
    ...
    role = models.CharField(max_length=20,choices=[(x,x.title()) for x in ['manager','caregiver','transport','frontdesk']])
    training = models.JSONField(default=list)
    class Meta:
        constraints=[models.UniqueConstraint(fields=['provider','user'],name='unique_staff')]
...
class Booking(models.Model):
    ...
    status = models.CharField(max_length=25,default='reserved',choices=[(x,x.title()) for x in ['reserved','confirmed','active','completed','cancelled']])
    ...
    class Meta:
        constraints=[models.CheckConstraint(condition=models.Q(ends__gt=models.F('starts')),name='valid_stay_dates')]
...
class CareTask(models.Model):
    ...
    def state(self):
        ...
        if self.escalated_at: return 'Escalated'
        if self.due_at < timezone.now(): return 'Late'
        return 'Upcoming'
...
class AppendOnly(models.Model):
    ...
    def save(self,*args,**kwargs):
        if self.pk: raise ValueError('Append-only record')
        super().save(*args,**kwargs)
    def delete(self,*args,**kwargs): raise ValueError('Append-only record')
...
class Incident(models.Model):
    booking = models.ForeignKey(Booking,on_delete=models.PROTECT,related_name='incidents')
...
class Trial(models.Model):
    ...
    provider=models.ForeignKey(Provider,on_delete=models.PROTECT)
    pet=models.ForeignKey(Pet,on_delete=models.PROTECT)
    kind=models.CharField(max_length=20,choices=[('video','Video call'),('meet','Meet and greet'),('daycare','Daycare trial'),('short_stay','Short stay')])
    scheduled_at=models.DateTimeField()
    outcome=models.CharField(max_length=30,default='pending',choices=[(x,x.replace('_',' ').title()) for x in ['pending','suitable','with_conditions','unsuitable']])
    owner_notes=models.TextField(blank=True)
    provider_notes=models.TextField(blank=True)

class CustodyEvent(AppendOnly):
    booking=models.ForeignKey(Booking,on_delete=models.PROTECT,related_name='custody_events')
    actor=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT)
    kind=models.CharField(max_length=30)
    details=models.JSONField(default=dict)

class Handover(models.Model):
    booking=models.OneToOneField(Booking,on_delete=models.PROTECT,related_name='handover')
...
class PrivacyRequest(models.Model):
    user=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT)
    kind=models.CharField(max_length=20,choices=[('export','Export'),('delete','Delete')])
    status=models.CharField(max_length=20,default='pending')
    created_at=models.DateTimeField(auto_now_add=True)
